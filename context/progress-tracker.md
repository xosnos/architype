...
- **Dashboard and project route migration (2026-08-26)**:
  ...
  - Renamed active workspace route props from `currentRoomId` to `currentProjectId`; Realtime and AI payload `roomId` fields remain unchanged because they identify collaboration channels rather than URL parameters.
...
- **Spec 30 implemented: Passwordless OTP Auth and Account Settings (2026-08-19)**:
  ...
  - Added in-app settings modal (Profile / Email, delete account OTP) opened from the user menu; `/settings` redirects to `/dashboard?settings=1`.
...
- **Spec 27 Acceptance Gap Resolution — Retained Integration Test Suite (2026-08-17)**:
  ...
  - Verified Group E (Metadata & Download APIs): `listProjectSpecs` queries metadata ordered by `created_at DESC` without leaking internal `file_path` storage paths; `getProjectSpec` enforces project boundary isolation; `downloadSpecMarkdown` retrieves exact stored Markdown; utilities (`parseSpecStoragePath`, `cleanMarkdownSpec`, `formatSpecFileName`).
...
- 28-spec-persistence-download — Spec Persistence, Metadata Queries, and Secure Download Route:
  - **Spec Metadata & Storage Access Layer (`lib/specs/queries.ts`, `types/specs.ts`)**:
    ...
    - Implemented `listProjectSpecs(supabase, projectId, projectName)` querying `project_specs` table with `id, task_run_id, project_id, created_at` (ordering: newest first `created_at DESC`), calculating clean user-facing filenames (`{project-name}-spec-{shortRunId}.md`) without leaking storage bucket paths.
    ...
    - Implemented `formatSpecFileName` generating standardized Markdown attachment filenames using full project name.
...
- Presence Avatars Navbar Integration & Workspace Icon Scoping:
  ...
  - **Workspace Navbar Icon Scoping**: Scoped the Community Discord, Twitter/X, and What's New (Gift) icons in `EditorNavbar` to only appear on the home dashboard (`/dashboard`, `!isWorkspace`), keeping the active canvas workspace clean and focused.
...
- Google Stitch Transparent Navigation Bar & User Nav Consolidation:
  ...
  - **Full-Bleed Canvas**: Updated `components/editor/editor-chrome.tsx` so the workspace canvas extends edge-to-edge full viewport behind the floating navbar header, while the dashboard (`/dashboard`) maintains clean top spacing (`pt-16`).
...
- Google Stitch Dashboard & Chrome Redesign — Modernized the dashboard (`/dashboard`) and chrome navigation to match Google Stitch's design language:
...
- 07-wire-editor-home — editor home sidebar and dialogs now use real project data. `app/dashboard/page.tsx` is a server component that fetches owned projects via `listOwnedProjects(user.id)` and shared projects via `listSharedProjects(user.email)` in parallel, then passes both lists + `currentUserId` through `EditorChrome` to `ProjectSidebar`. New `hooks/use-project-actions.ts` replaces the mock `use-project-dialogs.ts`: create calls `POST /api/projects` and navigates to `/projects/[projectId]` (project ID doubles as the Realtime channel name per spec 10); rename calls `PATCH` and `router.refresh()`; delete calls `DELETE` and redirects to `/dashboard` if deleting the active workspace, otherwise `router.refresh()`. All three dialogs now display server errors inline. `ProjectSidebar` accepts `ownedProjects`/`sharedProjects`/`currentUserId` props typed as `Project[]` and derives ownership from `project.ownerId === currentUserId` so rename/delete still appear only on owned items. Mock-data.ts and use-project-dialogs.ts deleted. `npm run build` passes.
- 08-editor-workspace-shell — `/projects/[projectId]` is a server component with server-side access checks. `lib/project-access.ts` provides `getAuthIdentity` (userId + email) and `hasProjectAccess` (owner or collaborator via `project_collaborators`). Unauthenticated users redirect to `/login`; missing or unauthorized projects render `AccessDenied` (lock icon, message, link back to `/dashboard`). Workspace layout extends `EditorChrome`: navbar shows project name plus Share and AI sidebar toggle buttons (no-op placeholders), `ProjectSidebar` highlights the current project and links to `/projects/[projectId]`, center area is a dark `CanvasPlaceholder`, right side is an `AiSidebarPlaceholder` that slides in from the right. No canvas, Supabase Realtime, AI chat, or sharing behavior yet. `npm run build` passes; route registers as dynamic (`ƒ /projects/[projectId]`).
...
- **Project routing stays UUID-based** per spec 07 ("project ID and room ID stay aligned"). The `/projects/[projectId]` route resolves projects by UUID; `POST /api/projects` returns the UUID and the client navigates to `/projects/<uuid>`. Slugs have been completely removed from dialogs, utilities, and routing; project names can exist with duplicates without conflict as UUIDs ensure project isolation.
...
- **Spec export filename generation format.** Generated Markdown spec download filenames format the project name in lowercase with hyphens (e.g. `{project-name}-spec-{shortRunId}.md` or `{project-name}-spec.md`), defaulting to `spec.md` if no project name is provided.
...
- Dashboard page at `/dashboard` is a server component that fetches the user, then renders client `EditorChrome` with the user email.
...
- Fixed `cookies was called outside a request scope` error on `/dashboard`. Root cause: each query function in `lib/projects/queries.ts` called `createClient()` (which calls `cookies()`) independently, and when `app/dashboard/page.tsx` ran two queries via `Promise.all`, the concurrent branches lost Next.js's async request-store context (`workUnitAsyncStorage`), so `cookies()` threw `throwForMissingRequestStore`. Fix: call `cookies()` exactly once per request to create a single Supabase client, then pass that client into `getCurrentUser(supabase?)` and all query functions (`listOwnedProjects(supabase, ownerId)`, `createProject(supabase, params)`, etc.). Updated the dashboard page and both API route handlers to use this shared-client pattern. Build passes; `/dashboard` remains dynamic (`ƒ`).
- Fixed `Failed to list shared projects: 'project' is not an embedded resource in this request` error on `/dashboard`. Two root causes: (1) the `project:...` embedded-resource join syntax in `listSharedProjects` was not recognized by PostgREST; (2) RLS policies on `projects` and `project_collaborators` only allowed project OWNERS to SELECT, so an invited collaborator could never read their shared projects. Fix: rewrote `listSharedProjects` to fetch matching `project_id`s from `project_collaborators` then fetch those project rows via `.in("id", projectIds)` (two simple queries instead of an embedded join); broadened both SELECT policies to allow owner OR collaborator (matched by `auth.jwt() ->> 'email'`) while keeping mutations owner-only. Applied via migration `fix_shared_projects_rls_and_access`. Build passes.
...
- Fixed post-auth redirect loop: call `router.refresh()` before `router.push("/dashboard")` so the proxy sees the session cookie; corrected the proxy `setAll` call to match `@supabase/ssr` (cookies only, no headers argument).
